...
Extend(Application, EventEmitter, {
  ...
  start: function(cb) {
    ...
    var initCount = 0;
    var timer = this._waitTimer(5000, 'Could not start in 5 seconds, shutting down...');
    this.modules.forEach(function(v) {
      this[v.name] = v.module;
      v.module.init(v.config, function() {
        initCount++;
        if (initCount >= this.modules.length) {
          this.emit('start');
          clearTimeout(timer);
          if (_.isFunction(cb)) {
            cb();
  ...
  shutdown: function shutdown(cb) {
    ...
    var shutdownCount = 0;
    var timer = this._waitTimer(5000, 'Could not shutdown in 5 seconds, hard shutting down...');
    this.modules.forEach(function(v) {
